...
void read_and_print_temperatures() {
    ...
    printf("[TEMP] Ambient: %.2f °C | Object: %.2f °C\n", temp_ambient_c, temp_object_c);
}
...
void read_and_print_radar() {
    ...
    while (uart_is_readable(RADAR_UART_ID)) {
        ...
        if (index >= 47) {
            ...
            printf("[RADAR] 1 dist: %.1f mm Angle: %.1f° Speed: %d mm/s X:%d Y:%d\n",
                   t1.distance, t1.angle, t1.speed, t1.x, t1.y);
            printf("[RADAR] 2 dist: %.1f mm Angle: %.1f° Speed: %d mm/s X:%d Y:%d\n",
                   t2.distance, t2.angle, t2.speed, t2.x, t2.y);
            printf("[RADAR] 3 dist: %.1f mm Angle: %.1f° Speed: %d mm/s X:%d Y:%d\n\n",
                   t3.distance, t3.angle, t3.speed, t3.x, t3.y);
        }
    }
}
...
int main() {
    ...
    while (1) {
        read_and_print_radar();  // Runs continuously (non-blocking UART read)
